...
try:
    ...
    from SGC.data.header import AudioConfig, AnimConfig, TextConfig, ActionConfig, make_sgc_header
    from SGC.data.package import SGCSocket
...
class SGCClient():
    def __init__(self, args = None, audio_path = None, anim_path = None, fps=15, port=12130):
        if args is not None:
            self.init_wth_args(args)
        else:
            self.init_without_args(audio_path, anim_path, fps, port)
    def init_without_args(self, audio_path, anim_path, fps, port) -> None:
        '''
        Initialize the sockets with default configs and audio and anim data input (used with hm gesturegen)
        '''
        self.all_blendshapes_sent = False
        self.all_audio_sent = False
        self.sample_chunk_size = int(16000/fps)+1
        self.frame_time = 1.0/fps
        self.bs_fps = fps
        self.stream_start_time = None
        self.audio_delay = 0
        self.blendshape_delay = 0
        self.max_time = 1e9
        self.socket = None

        self.send_audio = audio_path is not None
        self.send_anim = anim_path is not None
        ...
        self.socket = SGCSocket("localhost", port)

        # SET UP TEST FILES
        # open and format wave data
        self.wf = PyWave.open(self.get_16khz_audio_path(audio_path))
        # Format:
        #- WAVE_FORMAT_PCM (1)
        ...
        self.a2f_json_file = open(anim_path)
    ...
    def init_wth_args(self, args) -> None:
        '''
        ...
        # open and format wave data
        self.wf = PyWave.open(self.get_16khz_audio_path(audio_fpath))
        # Format:
        #- WAVE_FORMAT_PCM (1)
        #- WAVE_FORMAT_IEEE_FLOAT (3)
        #- WAVE_FORMAT_ALAW
    ...
    def get_16khz_audio_path(self, audio_fpath):
        data, orig_sr = librosa.load(audio_fpath, sr=None)
        if orig_sr == 16000:
            resampled_audio_fpath = audio_fpath
        else: # resample to 16khz
            resampled_audio_fpath = f'{audio_fpath[:-4]}_16khz.wav'
            if data.ndim == 1:
                # If mono, convert to 2D array for consistency
                data = data[np.newaxis, :]
            target_rate = 16000
            data_resampled = librosa.resample(data, orig_sr=orig_sr, target_sr=target_rate)
            sf.write(resampled_audio_fpath, data_resampled.T, target_rate)
        return resampled_audio_fpath
...
if __name__ == "__main__":
    ...
    default_audio_fpath = 'C:/Users/ARNO/Desktop/audio2pose/datasets/sample/actual-speech1.wav'
    #default_a2f_json_fpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), "bs_violet.json")
    default_a2f_json_fpath = 'C:/Users/ARNO/Desktop/audio2pose/datasets/sample/sgc-actual-speech1.json'
    ...
    sgc_client = SGCClient(args=args)
    sgc_client.initiate_data_transfer()
    sgc_client.close()
