...
from blendshapes import BLENDSHAPE_NAMES, SGC_BLENDSHAPE_NAMES
import json
import threading
...
from SGC.sgc_sender import SGCClient
...
class HMGestureGen():
    def __init__(self, blender_load_path, template_speaker = 17):
        ...
        self.multicontextnet.load_state_dict(torch.load('tmp/multicontextnet-no-text-normalized.pth', weights_only=True))
        self.multicontextnet = self.multicontextnet.cuda().eval()

        # set up camn
        print('Loading CaMN')
        self.camn = CaMN(self.camn_args)
        states = torch.load('tmp/camn.bin', weights_only=True)
    ...
    def load_gandhi_speech(self):
        self.audio_path = 'test_audio/gandhi-speech.wav'
        self.audio_data, self.orig_sr = librosa.load(self.audio_path, duration=30, sr=None) # np array
    
    def load_recording(self, file_path):
        self.audio_path = file_path
        y, sr = librosa.load(self.audio_path, sr=None)
        duration = int(len(y) / sr)
        self.audio_data, self.orig_sr = librosa.load(self.audio_path, sr=None, duration=duration)
    ...
    def bs2json_blender(self, facial, bvh_file):
    ...
    def save_for_blender(self):
        ...
        # generate json file
        self.bs2json_blender(self.pred_facial.tolist(), res_file)
    ...
    def save_sgc_json(self, pred_facial, fps=15): #TODO: facial working only
        sgc_json = {}
        bsNames = [name.capitalize() for name in BLENDSHAPE_NAMES[:pred_facial.shape[1]]]
        for index, entry in enumerate(pred_facial):
            sgc_json[str(index)] = {
                "Audio2Face": {
                    "Body": {},
                    "Facial": {
                        "Names": bsNames,
                        "Weights": entry.tolist(),
                    },
                    "FrameTiming": {
                        "FPS": fps,
                        "Index": index
        ...
        anim_file_path = 'ui_src/bs_gesturegen.json'
        with open(anim_file_path, 'w') as outfile:
            json.dump(sgc_json, outfile, indent=4)
    ...
    def send_to_ue5(self):
        ...
        fps = 15
        self.anim_path = self.save_sgc_json(self.pred_facial, fps)

        sgc_client = SGCClient(args=None, audio_path=self.audio_path, anim_path=self.anim_path, fps=fps, port=12130)
